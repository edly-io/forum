...
def test_pin_and_unpin_thread_api(
    api_client: Client,
    users_model: Users,
    comment_thread_model: CommentThread,
    ...
    comment_thread_id = comment_thread_model.insert(
        title="title",
        body="Hello World!",
        pinned=False,
        author_id=user_id,
    ...
    mock_thread_class = Mock(return_value=comment_thread_model)
    mock_comment_class = Mock(return_value=comment_model)
    ...
    with patch("forum.models.Users", new=mock_users_class):
        with patch("forum.models.CommentThread", new=mock_thread_class):
            with patch("forum.models.Comment", new=mock_comment_class):
                ...
                thread = comment_thread_model.get(comment_thread_id)
...
def test_pin_unpin_thread_api_invalid_data(
    api_client: Client, users_model: Users, comment_thread_model: CommentThread
) -> None:
    """
